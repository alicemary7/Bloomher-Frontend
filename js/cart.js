...
// COMMAND: Fetch Cart Items - Loads user's cart from API
...
// COMMAND: Update Quantity - Updates item quantity in cart
async function updateQuantity(productId, newQty) {
    ...
    try {
        const response = await fetch(`${CART_API_URL}/item/${productId}?quantity=${newQty}`, {
            method: "PUT",
            headers: { "Authorization": `Bearer ${token}` }
        });
...
// COMMAND: Remove Cart Item - Deletes item from user's cart
...
// COMMAND: Proceed to Checkout - Prepares for checkout and redirects
