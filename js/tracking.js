...
// COMMAND: animateProgressBar - Visual status bar update
...
// COMMAND: init - Setup tracking session
function init() {
    if (!token) {
...
// COMMAND: handleSearch - Fetch and reveal order progress
async function handleSearch() {
    ...
    try {
        ...
        const orderDateStr = date.toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' });

        displayOrderId.innerHTML = (ids.length > 1 ? `Order IDs: #${ids.join(", #")}` : `Order ID: #${ids[0]}`) +
...
// COMMAND: renderOrdersSummary - Display items and totals
function renderOrdersSummary(orders) {
    ...
    orderShippingEl.textContent = shipping < 0.01 ? "FREE" : `₹${shipping.toFixed(2)}`;
...
// COMMAND: updateTrackingUI - Update status text and progress
